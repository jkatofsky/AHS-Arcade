...
class Saucer {
  ...
  shoot() {
    if (this.stupid)
      ...
      var laser = new Lazer(this.loc.copy(), p5.Vector.sub(ship.loc, this.loc));
      laser.velocity.setMag(10);
      laser.velocity.rotate(radians(Math.random() * 40 - 20));
      this.lazers.push(laser);
    }
...
function play() {
  ...
  if (state === STARTSCREEN) {
    ...
    textSize(20);
    text("press ENTER to sumbit your score", width / 2, height / 2 + 35);
    text("or press BACKSPACE to discard it", width / 2, height / 2 + 60);
    textSize(25);
    text(to6digits(points), width - 150, 50);
  }
}
